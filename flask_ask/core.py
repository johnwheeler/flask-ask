...
class Ask(object):
    ...
    def init_app(self, app):
        ...
        self.ask_verify_timestamp_debug = app.config.get(
    ...
    def on_playback_nearly_finished(self, mapping={'offset': 'offsetInMilliseconds'}, convert={}, default={}):
        ...
        **Note** that this request is sent when Alexa is ready to receive a new stream to enqueue, and not
        ...
        def decorator(f):
            ...
            self._intent_mappings[
                'AudioPlayer.PlaybackNearlyFinished'] = mapping
            self._intent_converts[
                'AudioPlayer.PlaybackNearlyFinished'] = convert
            self._intent_defaults[
                'AudioPlayer.PlaybackNearlyFinished'] = default
    ...
    def _alexa_request(self, verify=True):
        ...
        if verify:
            ...
            # load certificate - this verifies a the certificate url and format
            # under the hood
            ...
            try:
                application_id = alexa_request_payload[
            ...
            if self.ask_application_id is not None:
                verifier.verify_application_id(
                    application_id, self.ask_application_id)
    ...
    def _map_player_request_to_func(self, player_request_type):
        ...
        view_func = self._intent_view_funcs.get(
            player_request_type, lambda: None)

        argspec = inspect.getargspec(view_func)
        arg_names = argspec.args
        arg_values = self._map_params_to_view_args(
            player_request_type, arg_names)
