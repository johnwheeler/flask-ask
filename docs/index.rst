...
Building Responses
==================
We wrote code to listen to Echo in the last section. Now let's make Echo talk back. The two primary constructs in
Flask-Ask for creating responses are ``statement`` and ``question``.
...
`ASK_VERIFY_TIMESTAMP`  Turn on request timestamp verification by setting this to ``True``. Timestamp verification
                        helps mitigate against `replay attacks <https://en.wikipedia.org/wiki/Replay_attack>`_. It
                        relies on the system clock being synchronized with an NTP server. By default, this setting
                        is disabled and a warning is logged. It should be enabled in production.
                        **Default:** ``False``
======================= ============================================================================================
